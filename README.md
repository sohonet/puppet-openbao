...
Puppet module to install and run [OpenBao](https://openbao.org).
...
Please see [The official documentation](https://www.openbao.org/docs/configuration/index.html) for further details of acceptable parameter values.

## Parameters

### Setup parameters

* `user`: Customise the user bao runs as, will also create the user unless `manage_user` is false.
* `manage_user`: Whether or not the module should create the user.
* `group`: Customise the group bao runs as, will also create the user unless `manage_group` is false.
* `manage_group`: Whether or not the module should create the group.
* `bin_dir`: Directory the bao executable will be installed in.
* `config_dir`: Directory the bao configuration will be kept in.
* `config_mode`: Mode of the configuration file (openbao.hcl). Defaults to '0750'
...
* `service_options`: Extra argument to pass to `bao server`, as per: `bao server --help`
...
* `manage_config_file`: Manages the configuration file. When set to false, `openbao.hcl` will not be generated. `manag_storage_dir` is ignored. Default: `true`
...
* `manage_file_capabilities`: Will manage file capabilities of the bao binary. default: `false`
...
By default, with no parameters the module will configure openbao with some sensible defaults to get you running, the following parameters may be specified to configure OpenBao.  Please see [The official documentation](https://www.openbao.org/docs/configuration/index.html) for further details of acceptable parameter values.
...
* `enable_ui`: Enable the openbao UI (requires openbao 2.1.0+) (default: `undef`)
...
When running OpenBao in agent mode (`mode => 'agent'`), you can configure the agent behavior using these parameters:
...
class { 'openbao':
  ...
  agent_template => {
    'source'      => '/etc/openbao/template.ctmpl',
